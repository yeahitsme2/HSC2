...
{
    // Wait for IRC to get it first - disabled with HSC
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "HSC " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are HSC dns seeds.
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  HSC is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // HSC: mint proof-of-stake blocks in the background
