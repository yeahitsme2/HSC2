...
// HSC: optional setting to unlock wallet for block minting only;
...
// HSC: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // HSC: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // HSC: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // HSC: not avatar mode
                    {
...
// HSC: create coin stake transaction
...
// HSC: check 'spent' consistency between wallet and txindex
// HSC: fix wallet spent state according to txindex
...
// HSC: disable transaction (only for coinstake)
