...
static const int STAKE_START_TIME = 1408636880;
...
public:
    ...
    {
        // HSC: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    int64 GetMinFee(unsigned int nBlockSize=1, bool fAllowFree=false, enum GetMinFee_mode mode=GMF_BLOCK) const
    {
        ...
        if (fAllowFree)
        {
            if (nBlockSize == 1)
            {
                // Transactions under HSC are free
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // HSC: get transaction coin age
...
public:
    ...
    // HSC: block signature - signed by coin base txout[0]'s owner
    ...
    // HSC: two types of block: proof-of-work or proof-of-stake
    ...
    // HSC: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // HSC: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // HSC: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // HSC: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // HSC: block index flags
    enum  
    {
